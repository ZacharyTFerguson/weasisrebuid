...
**HEAD:** `14d91f0` (`Show dicom:close live named close-state chrome.`). **T/W left-mouse pan ≠ winLevel** / `name=left-state` headed PASS on `2b25a0d` (Composer: mouse_left pan≠winLevel on ViewerToolBar **bound view**, NOT cal-view). **S/Z left-mouse sequence ≠ zoom** / `name=sz-state` headed PASS on `4acbfef` (Composer: chest `org.weasis.dicom.viewer2d.View2d` bound ViewerToolBar, NOT cal-view; cal-view `mouse_left` stays `winLevel`; `left-state` stays `none`). **R/Q left-mouse rotation ≠ contextMenu** / `name=rq-state` headed PASS on `b1e7b15` (Composer: chest bound ViewerToolBar, NOT cal-view; cal-view `mouse_left` stays `winLevel`; `left-state` / `sz-state` stay `none`). **Alt+R/L/F quarter-rotate ≠ flip** / `name=alt-state` headed PASS on `0136004` (Composer: chest bound ViewerToolBar; `alt-state` none→90→0→flip; cal-view rotation 0 / flip false; `rq-state` / `sz-state` / `left-state` stay `none`). **P ImagePrint** `94405ae`, **Freeze** `4dd370e`, **Calibration** `f8f66da`, **Q/R** `a17cff1`, **ISO** `693601d`, **DICOM Print** `ea11210`, **send** `090fa16`, and prior chrome headed PASS (not re-scored). **dicom:close series ≠ all** / `name=close-state` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass. CHECKLIST §6 Pass stays **empty**.

Full ledger: `docs/agent_pr25_body_14d91f0_inprogress.md`. Sprint inventory: `docs/weasis-spec/TUTORIALS-SPRINT-INVENTORY.md`.
...
**Jar:** `/tmp/weasis-headed-14d91f0.jar` · **SHA:** `14d91f066ddc66b42e767e902ed667189323644c` · **Jar file:** `d02da07ee3bf829f69fb4d9632c58a3f0a404731b60968ec0dda34512f44506f` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-14d91f0-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-0136004.jar` or Alt/R/Q/S/Z/T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`close-series`**, **`close-all`**, **`close-state`** on **DICOM Explorer** (`DicomExplorer` — same dock as `explorer-series` / `explorer-filter-hits` / `apply-pr`; **not** ViewerToolBar, **not** `cal-view`, **not** `window-close` / `help-shortcuts-close` / docking Ctrl+W). Live `name=`, `doClick`, not OCR. After load, **`close-state`** `getText` expect `none` (chest series present; `filterHits` ≥ 1). Click **`close-series`** expect `close-state` `series` (that series gone from explorer; **`alt-state` / `rq-state` / `sz-state` / `left-state` still `none`**). Click **`close-all`** expect `close-state` `all` (model empty; series ≠ all). Ordered: series then all. Do **not** click `window-close` / `help-shortcuts-close` / `apply-pr` / `apply-interp` / `left-r` / `alt-r` / RotationToolBar `90°`. Confirm `alt-state` / `rq-state` / `sz-state` / `left-state` / `img-print-state` / `print-state` / `freeze-state` / `cal-state` / `qr-state` / `iso-state` / `send-state` still `none`; `graphics-count` still `0`. No PACS / Print SCP / ISO burn / PrinterJob. Series DnD CLocation split **not** this item (blocked: spec series `dragging()` does not split).

| Check | Stills-based result |
|---|---|
| DicomExplorer `close-series` / `close-all` / `close-state` (NOT cal-view ViewerToolBar) | **pending** — `name=close-state`; after Reed load `close-state` `none`; explorer has chest series |
| `dicom:close -s` series | **pending** — after `close-series`, `close-state` `series`; chest series gone; `alt-state` / `rq-state` / `sz-state` / `left-state` still `none` |
| `dicom:close -a` all ≠ series | **pending** — after `close-all`, `close-state` `all` (series ≠ all) |
| alt-state / rq-state / sz-state / left-state / img-print / print / freeze / cal / qr / iso / send / graphics-count untouched | **pending** — close-only jattach; prior chrome not re-scored |

**Route:** Composer headed verify **in progress** on `14d91f0`. **Next:** remaining TUTORIALS after dicom:close — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_14d91f0_close.png`
