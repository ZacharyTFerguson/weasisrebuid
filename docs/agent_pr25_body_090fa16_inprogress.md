...
|---|---|
| WeasisWin `send-cstore` / `send-stow` / `send-state` | **pending** — `name=send-state`; after Reed load `send-state` `getText` `none` |
| C-STORE | **pending** — after `send-cstore`, `send-state` `C-STORE` (jattach `getText`, not OCR) |
| STOW-RS | **pending** — after `send-stow`, `send-state` `STOW-RS` |
| sel-state / graphics-count / voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — send-only jattach; `sel-state` still `none`; `graphics-count` still `0`; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** Composer headed verify **in progress** on `090fa16`. **Next:** remaining TUTORIALS after send C-STORE/STOW — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_090fa16_send.png`
