...
**HEAD:** `4c6297c` (`Show i18n lang-pref live named lang-state chrome.`). **dicom:close series ≠ all** / `name=close-state` headed PASS on `14d91f0` (Composer: `org.weasis.dicom.explorer.DicomExplorer` `close-series`/`close-all`; `close-state` none→series→all; NOT `window-close`). **Alt+R/L/F** / `name=alt-state` headed PASS on `0136004`. **R/Q** `b1e7b15`, **S/Z** `4acbfef`, **T/W** `2b25a0d` headed PASS (chest ViewerToolBar, NOT cal-view). **P ImagePrint** `94405ae`, **Freeze** `4dd370e`, **Calibration** `f8f66da`, **Q/R** `a17cff1`, **ISO** `693601d`, **DICOM Print** `ea11210`, **send** `090fa16`, and prior chrome headed PASS (not re-scored). **i18n lang-pref en ≠ fr** / `name=lang-state` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass. CHECKLIST §6 Pass stays **empty**.

Full ledger: `docs/agent_pr25_body_4c6297c_inprogress.md`. Sprint inventory: `docs/weasis-spec/TUTORIALS-SPRINT-INVENTORY.md`.
...
**Jar:** `/tmp/weasis-headed-4c6297c.jar` · **SHA:** `4c6297c28e2011b614338f06ac90af804f8ca029` · **Jar file:** `9df37a10da9211da2e0f4e6b0738c313791053d080194947cabf715d3bd64ba7` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-4c6297c-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-14d91f0.jar` or close/Alt/R/Q/S/Z/T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`lang-en`**, **`lang-fr`**, **`lang-state`** on **WeasisWin north bar** (live `name=`, `doClick`, not OCR) — **not** ViewerToolBar, **not** `cal-view`, **not** `file-preferences` / `pref-ok` / `close-all` / `alt-r` / `90°`. After load, **`lang-state`** `getText` expect `none`. Click **`lang-en`** expect `lang-state` `en` (`locale.lang.code` en; **`close-state` / `alt-state` / `rq-state` / `sz-state` / `left-state` still `none`**). Click **`lang-fr`** expect `lang-state` `fr` (`locale.lang.code` fr; en ≠ fr). Documented keys only: `locale.lang.code` (LanguageSetting `lang-list`/`lang-percent` at `weasis.i18n.min.percent` 30% lists en and fr). Do **not** invent preference keys. Do **not** click `file-preferences` / `pref-ok` / `close-series` / `close-all` / `window-close` / `help-shortcuts-close` / `alt-r` / `left-r` / RotationToolBar `90°`. Confirm `close-state` / `alt-state` / `rq-state` / `sz-state` / `left-state` / `img-print-state` / `print-state` / `freeze-state` / `cal-state` / `qr-state` / `iso-state` / `send-state` still `none`; `graphics-count` still `0`. No PACS / Print SCP / ISO burn / PrinterJob. Series DnD CLocation split **not** this item (blocked). Dicomizer **not** this item (separate overlay). Cine / AU/ECG/SR/RT / Q/R calling-AE skipped.

| Check | Stills-based result |
|---|---|
| WeasisWin `lang-en` / `lang-fr` / `lang-state` (NOT cal-view ViewerToolBar / NOT file-preferences) | **pending** — `name=lang-state`; after Reed load `lang-state` `none` |
| lang-en | **pending** — after `lang-en`, `lang-state` `en`; `locale.lang.code` `en`; close/alt/rq/sz/left still `none` |
| lang-fr ≠ lang-en | **pending** — after `lang-fr`, `lang-state` `fr` (en ≠ fr) |
| close-state / alt-state / rq-state / sz-state / left-state / img-print / print / freeze / cal / qr / iso / send / graphics-count untouched | **pending** — lang-only jattach; prior chrome not re-scored |

**Route:** Composer headed verify **in progress** on `4c6297c`. **Next:** remaining TUTORIALS after i18n lang-pref — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_4c6297c_lang.png`
