...
|---|---|
| Toolbar `save-ko` / `save-pr` / `ko-uid` / `pr-uid` | **pending** — `name=save-ko` / `save-pr`; after Reed load `ko-uid` / `pr-uid` `getText` `none` |
| KO SOP UID root 2.25 | **pending** — after `save-ko`, `ko-uid` `getText` starts with `2.25` |
| PR SOP UID root 2.25 | **pending** — after `save-pr`, `pr-uid` `getText` starts with `2.25` |
| ko-state / graphics-count / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — KO/PR-uid-only jattach; `ko-state` still `none`; `graphics-count` still `0`; `region-stats` named; `display-visibility-value` `FULL`; `docking-state` `NORMAL`; `synch-kind` `FoR`; docks unchanged |

**Route:** Composer headed verify **in progress** on `a781895`. **Next:** remaining TUTORIALS after KO/PR objects — **not** Pass.
