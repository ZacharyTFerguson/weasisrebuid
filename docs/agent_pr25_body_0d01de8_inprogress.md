...
**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. Read toolbar **`docking-state`** `getText` (not OCR) expect `NORMAL`. Click **`window-maximize`** expect `MAXIMIZED`. Click **`window-normalize`** expect `NORMAL`. Click **`window-externalize`** expect `EXTERNALIZED`. Click **`window-normalize`** restore `NORMAL`. Click **`window-docking-list`**. Dialog **`docking-list`** title `Docking List`; read **`docking-list-items`** `getText` contains `DICOM 2D`. Close **`docking-list-close`**. Confirm `synch-kind` still `FoR`, `lens` east, `mini-tool` west, `histogram` south, `pixel-info` on Viewer. Do **not** click `window-close` / lens sliders / `synch-manual` / `crosshair` / Mini Tool / Histogram RGB / Preferences / About / Import / Screenshot / Help / Export / ImageTool / `dumpHeader` / Fields.

| Check | Stills-based result |
|---|---|
| Toolbar `window-maximize` / `window-normalize` / `window-externalize` | **pending** — named chrome; `docking-state` `NORMAL` → `MAXIMIZED` → `NORMAL` → `EXTERNALIZED` → `NORMAL` |
| Dialog `docking-list` | **pending** — title `Docking List`; `docking-list-items` `getText` includes `DICOM 2D`; close `docking-list-close` |
| FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — docking-keys-only jattach; `synch-kind` `FoR`; `lens` east; `mini-tool` west; `histogram` south; `pixel-info` present |

**Route:** Composer headed verify **in progress** on `0d01de8`. **Next:** remaining TUTORIALS after docking keys — **not** Pass.
