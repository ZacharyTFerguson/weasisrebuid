...
|---|---|
| Viewer `graphics-pane` / `graphics-count` | **pending** — `name=graphics-pane`; after Reed load `graphics-count` `getText` `0` |
| Bound drawing count after G | **pending** — after `measure-rect` click-drag, `graphics-count` `getText` `1` |
| region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — GraphicsPane-only jattach; `display-visibility-value` `FULL`; `docking-state` `NORMAL`; `synch-kind` `FoR`; docks unchanged |

**Route:** Composer headed verify **in progress** on `3699b45`. **Next:** remaining TUTORIALS after GraphicsPane — **not** Pass.
