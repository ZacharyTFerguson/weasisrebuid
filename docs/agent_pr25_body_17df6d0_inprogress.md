...
**HEAD:** `17df6d0` (`Show Help About live version dialog with named Weasis 4.7.3 chrome.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, **Mask** `3355cda`, **Shutter** `b00cd4d`, **DcmHeader Dump** `0d2f39f`, **DicomFieldsView** `eefe975`, **Export DICOM** `363f172`, **Help shortcuts/resources** `e88e780`, **Screenshot** `fc115d3`, and **Import DICOM** `a569b23` headed PASS (prior chrome not re-scored). **Help About** / `name=help-about` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
|---|---|
| Help `help-about` / `About` | **pending** — dialog title `About Weasis`; `name=about` |
| Version `about-version` | **pending** — `getText` `Weasis 4.7.3` |
| Note `about-note` | **pending** — `Clean-room rebuild`; close `help-about-close` |
| Import / Screenshot / Export / ImageTool / Dump / Fields untouched | **pending** — About-only jattach; no Import/Screenshot/Export/`dumpHeader`/Fields clicks |

**Route:** Composer headed verify **in progress** on `17df6d0`. **Next:** remaining TUTORIALS after About — **not** Pass.
