...
|---|---|
| Dock `mini-tool` / Mini Tool | **pending** — `name=mini-tool` west of DICOM 2D; sliders `mini-zoom` / `mini-rotation` / `mini-series` |
| Zoom `mini-zoom` → `200%` | **pending** — `mini-zoom-value` `getText` `200%` after set 200 (paint-time zoom) |
| Panner `mini-panner` | **pending** — named overview navigator present |
| Histogram / Preferences / About / Import / Screenshot / Export / ImageTool / Dump / Fields untouched | **pending** — Mini Tool-only jattach; `histogram` still south |

**Route:** Composer headed verify **in progress** on `5497cf5`. **Next:** remaining TUTORIALS after Mini Tool — **not** Pass.
