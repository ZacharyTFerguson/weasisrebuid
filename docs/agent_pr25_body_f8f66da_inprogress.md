...
|---|---|
| WeasisWin `cal-line` / `cal-apply` / `cal-state` | **pending** — `name=cal-state`; after Reed load `cal-state` `getText` `none` |
| Distance sample | **pending** — after `cal-line`, `cal-state` `line` (jattach `getText`, not OCR); graphics-count still `0` |
| Session mm/pixel ≠ monitor pitch (MX-07) | **pending** — after `cal-apply`, `cal-state` `session`; session 2.0 mm/pixel; monitor pitch 0.2 |
| qr-state / iso-state / print-state / send-state / sel-state / graphics-count / voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — Calibration-only jattach; `qr-state` still `none`; `iso-state` still `none`; `print-state` still `none`; `send-state` still `none`; `sel-state` still `none`; `graphics-count` still `0`; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** Composer headed verify **in progress** on `f8f66da`. **Next:** remaining TUTORIALS after CalibrationView — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_f8f66da_cal.png`
