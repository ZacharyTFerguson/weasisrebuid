...
|---|---|
| Dock `histogram` / Histogram | **pending** — `name=histogram` south of DICOM 2D; panel `histogram-panel` |
| Stats `histogram-stats` | **pending** — `getText` starts `n=` (full Reed raster after import) |
| RGB chrome `histogram-rgb` | **pending** — click RGB; channels `histogram-channels` visible |
| Preferences / About / Import / Screenshot / Export / ImageTool / Dump / Fields untouched | **pending** — Histogram-only jattach |

**Route:** Composer headed verify **in progress** on `9d00ffa`. **Next:** remaining TUTORIALS after Histogram — **not** Pass.
