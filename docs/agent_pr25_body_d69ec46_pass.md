...
**Jar:** `/tmp/weasis-headed-d69ec46.jar` · **SHA:** `d69ec4681cc8cf438ccda1c9d20c8c6045f37b69` · **Jar file:** `6e0d38a1b2f54da3b8f76f4635c8cbe44c0ddb2455108b1464e58bebb402e3df` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-d69ec46-fresh`.

**Flow:** `dicom:get` Reed chest on **DICOM 2D**. jattach on **`org.weasis.dicom.explorer.DicomExplorer`**: `rs-state` `none` → `rs-qido` → `qido` → `rs-wado` → `wado-rs` (no PACS). `lang-state` / `close-state` / `alt-state` / `rq-state` / `sz-state` / `left-state` stay `none`.

| Check | Stills-based result |
|---|---|
| DicomExplorer `rs-qido` / `rs-wado` / `rs-state` | **OK** — explorer class `org.weasis.dicom.explorer.DicomExplorer` |
| QIDO plan | **OK** — after `rs-qido`, `rs-state` `qido` |
| WADO-RS ≠ QIDO | **OK** — after `rs-wado`, `rs-state` `wado-rs` |
| lang/close/alt/rq/sz/left untouched | **OK** — all `none`; `graphics-count` `0` |

**Route:** dicom:rs QIDO ≠ WADO-RS / `rs-state` **headed OK** on `d69ec46`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_d69ec46_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_d69ec46_rs.png" width="720" />
