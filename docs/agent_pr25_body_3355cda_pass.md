...
**HEAD:** `3355cda` (`Add ImageTool Mask chrome that blackens outside the center via MaskOp.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, and **AutoLevels** `18f9802` headed PASS (prior chrome not re-scored). **Mask** `3355cda` headed PASS. **Next:** Shutter — **not** Pass. Full TUTORIALS.md walkthrough is still **not** Pass.
