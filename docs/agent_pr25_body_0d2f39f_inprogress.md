...
**HEAD:** `0d2f39f` (`Show DcmHeader Dump of the painted View2d dataset in a live text pane.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, **Mask** `3355cda`, and **Shutter** `b00cd4d` headed PASS (ImageTool 2D chrome not re-scored). **DcmHeader Dump** / `dumpHeader` + `headerDump` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
**Flow:** `dicom:get` Reed chest on **DICOM 2D** → jattach **`dumpHeader`** (DICOM Header toolbar) → click **Dump** → read **`headerDump`** `JTextArea` via jattach (not OCR) + bound View2d still **1163,561,134×364**.

| Check | Stills-based result |
|---|---|
| 2D tab with chest loaded | **pending** — headed run |
| **`dumpHeader` / Dump** on DICOM Header chrome | **pending** — jattach `name=dumpHeader` |
| **`headerDump`** shows Reed tags | **pending** — PatientName/ID, Modality DX, WW 65535, Rows/Columns 1929/2207, PixelData `[OW]` |
| ImageTool shutter/mask/flip/filter untouched | **pending** — raster **1929×2207** |

**Route:** Composer headed verify running on `0d2f39f`. **Next:** explorer DicomFieldsView / cine — **not** Pass.
