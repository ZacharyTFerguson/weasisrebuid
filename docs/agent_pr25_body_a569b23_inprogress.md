...
**HEAD:** `a569b23` (`Show File/toolbar Import DICOM of a Part-10 path in a live dialog.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, **Mask** `3355cda`, **Shutter** `b00cd4d`, **DcmHeader Dump** `0d2f39f`, **DicomFieldsView** `eefe975`, **Export DICOM** `363f172`, **Help shortcuts/resources** `e88e780`, and **Screenshot** `fc115d3` headed PASS (prior chrome not re-scored). **Import DICOM** / `name=import-dicom` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
**Jar:** `/tmp/weasis-headed-a569b23.jar` · **SHA:** `a569b23716c8dba6c189e00c623088155d568947` · **Jar file:** `32c308b950e5a7e069ad9c1863b568c89145aeb35947e4958009453eae7f1d5e` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-a569b23-fresh`; prior headed JVMs killed; **did not reuse** Screenshot/Help/Export/Fields/Dump/ImageTool jars.
...
|---|---|
| Toolbar `import-dicom` / File `file-import-dicom` | **pending** — jattach `name=import-dicom` opens `import-dicom-dialog` title `Import DICOM` |
| Tabs DICOM / ZIP / DICOMDIR | **pending** — `import-tabs` titles **DICOM** / **ZIP** / **DICOMDIR** |
| `import-run` loads dest; `import-status` Imported | **pending** — status starts `Imported `; explorer Reed^Alice XR-CHEST-001 |
| Screenshot / Help / Export / ImageTool / Dump / Fields untouched | **pending** — raster **1929×2207** after import; Screenshot/Help/Export not clicked |

**Route:** Composer headed verify **in progress** on `a569b23`. **Next:** remaining TUTORIALS after Import — **not** Pass.
