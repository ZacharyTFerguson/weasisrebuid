...
|---|---|
| Dock `lens` / Lens | **pending** — `name=lens` east of DICOM 2D; slider `lens-factor`; preview `lens-panel` |
| Factor `lens-factor` → `4x` | **pending** — `lens-factor-value` `getText` `2x` default; `4x` after jattach set `lens-factor` 400 |
| PixelInfo / Mini Tool west / Histogram south untouched | **pending** — Lens-only jattach; `mini-tool` west; `histogram` south; `pixel-info` present |

**Route:** Composer headed verify **in progress** on `ceb0de9`. **Next:** remaining TUTORIALS after lens — **not** Pass.
