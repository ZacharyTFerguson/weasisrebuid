...
**Jar:** `/tmp/weasis-headed-0136004.jar` · **SHA:** `01360042956f3f75eae05008a74b9557334a4e4e` · **Jar file:** `eb18e97de5c2762162c8bdd89833aec76e081c6d1b8e7079207bd088ab21b7de` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-0136004-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-b1e7b15.jar` or R/Q/S/Z/T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`alt-r`**, **`alt-l`**, **`alt-f`**, **`alt-state`** on **ViewerToolBar** whose `boundView()` is `org.weasis.dicom.viewer2d.View2d` (View2dContainer DICOM 2D bar — **not** WeasisWin north bar bound to `cal-view`). Duplicate names may exist on the north bar; pick the bar whose `boundView()` is the chest View2d (same instance as R/Q `left-r` / `left-q`). Live `name=`, `doClick`, not OCR. After load, **`alt-state`** `getText` expect `none`. Click **`alt-r`** expect `alt-state` `90` (chest rotation 90; cal-view rotation 0; flip false; **`rq-state` / `sz-state` / `left-state` still `none`**). Click **`alt-l`** expect `alt-state` `0` (chest rotation 0; 90 ≠ 0; cal-view still 0). Click **`alt-f`** expect `alt-state` `flip` (chest flip true; cal-view flip false). Do **not** click RotationToolBar `90°` / `0°` / `reset-rotation` / ACTION `rotation` / `left-r` / `left-q` / `left-t` / `left-w` / `left-s` / `left-z` / `window-close` / `inverseLut`. Confirm `rq-state` / `sz-state` / `left-state` / `img-print-state` / `print-state` / `freeze-state` / `cal-state` / `qr-state` / `iso-state` / `send-state` still `none`; `graphics-count` still `0`. No PACS / Print SCP / ISO burn / PrinterJob.

| Check | Stills-based result |
|---|---|
| ViewerToolBar `alt-r` / `alt-l` / `alt-f` / `alt-state` on chest bound view (NOT cal-view) | **pending** — `name=alt-state`; after Reed load `alt-state` `none`; `boundView()` is chest `org.weasis.dicom.viewer2d.View2d` |
| Alt+R +90 | **pending** — after `alt-r`, `alt-state` `90`; chest rotation 90; cal-view 0; `rq-state` / `sz-state` / `left-state` still `none` |
| Alt+L −90 ≠ Alt+R +90 | **pending** — after `alt-l`, `alt-state` `0`; chest rotation 0 (90 ≠ 0); cal-view still 0 |
| Alt+F flip | **pending** — after `alt-f`, `alt-state` `flip`; chest flip true; cal-view flip false |
| rq-state / sz-state / left-state / img-print / print / freeze / cal / qr / iso / send / graphics-count untouched | **pending** — Alt+R/L/F-only jattach; prior chrome not re-scored |

**Route:** Composer headed verify **in progress** on `0136004`. **Next:** remaining TUTORIALS after Alt+R/L/F — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_0136004_alt.png`
