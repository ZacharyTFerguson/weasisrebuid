...
**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. jattach Viewer **`synch-manual`**. Read **`synch-kind`** `getText` (not OCR) expect `Manual`. Click **`synch-for`** expect `FoR`. Confirm `lens` still east, `mini-tool` west, `histogram` south, and `pixel-info` still on Viewer. Do **not** click lens sliders / `crosshair` / Mini Tool / Histogram RGB / Preferences / About / Import / Screenshot / Help / Export / ImageTool / `dumpHeader` / Fields.

| Check | Stills-based result |
|---|---|
| Viewer `synch-for` / `synch-manual` | **pending** — `name=synch-for` FoR; `name=synch-manual` Manual |
| `synch-kind` `getText` | **pending** — `FoR` default; `Manual` after jattach click `synch-manual`; restore `synch-for` → `FoR` |
| Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — FoR/manual-only jattach; `lens` east; `mini-tool` west; `histogram` south; `pixel-info` present |

**Route:** Composer headed verify **in progress** on `0e7a548`. **Next:** remaining TUTORIALS after FoR vs manual — **not** Pass.
