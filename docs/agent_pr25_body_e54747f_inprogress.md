...
|---|---|
| Viewer `crosshair` | **pending** — `name=crosshair` selected after click (H) |
| `pixel-info` `getText` | **pending** — contains `x,y` and `v=` after click on Reed |
| Mini Tool west / Histogram south untouched | **pending** — PixelInfo-only jattach; `mini-tool` west; `histogram` south |

**Route:** Composer headed verify **in progress** on `e54747f`. **Next:** remaining TUTORIALS after 3D cursor — **not** Pass.
