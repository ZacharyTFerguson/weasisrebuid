...
|---|---|
| Toolbar `key-object` / `star` / `ko-filter` / `ko-state` | **pending** — `name=key-object`; after Reed load `ko-state` `getText` `none` |
| Star / Filter cycle | **pending** — `star` → `starred`; `ko-filter` → `filtered`; `ko-filter` off → `starred`; `star` off → `none` |
| `star` vs Lut `filter` distinct | **pending** — KO Filter is `ko-filter`; Lut Sharpen stays `name=filter` (not clicked) |
| graphics-count / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — KO-only jattach; `graphics-count` still `0`; `region-stats` named; `display-visibility-value` `FULL`; `docking-state` `NORMAL`; `synch-kind` `FoR`; docks unchanged |

**Route:** Composer headed verify **in progress** on `4bf92c6`. **Next:** remaining TUTORIALS after Key Object Star/Filter — **not** Pass.
