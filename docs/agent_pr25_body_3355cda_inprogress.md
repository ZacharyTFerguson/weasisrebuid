...
**HEAD:** `3355cda` (`Add ImageTool Mask chrome that blackens outside the center via MaskOp.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, and **AutoLevels** `18f9802` headed PASS (prior chrome not re-scored). **Mask** / `name=mask` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
| **`mask` / Mask** on Image chrome | **pending** — jattach `name=mask` |
| Live toggle **Mask** on (full raster; not Crop half) | **pending** — source **1929×2207**; outer blacken; mean \|Δ\| ~**100**; corr ~**0.11** |
| Live toggle **Mask** off (restore) | **pending** — corr **1.0**; mean \|Δ\| **0** |
| **autolevels** / **brightness** / **crop** / **window** / **flip** / **filter** untouched | **pending** |

**Route:** Composer headed verify running on `3355cda`. **Next:** Shutter after Pass — **not** Pass.
...
| **`autolevels` / AutoLevels** on Image chrome | **OK** — `1065,470,109×25`; **brightness** `954,470,107×25`; **mask** (between autolevels and flip); **flip** (right of mask) |
