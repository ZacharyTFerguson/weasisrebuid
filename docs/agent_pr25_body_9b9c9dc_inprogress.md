...
|---|---|
| Explorer `apply-pr` / `pr-graphics` / `pr-mapped` | **pending** — `name=apply-pr`; after Reed load `pr-graphics` `0` and `pr-mapped` `none` |
| GSPS POLYLINE → Weasis Graphic | **pending** — after `apply-pr`, `pr-graphics` `1` and `pr-mapped` `LineGraphic` (jattach `getText`, not OCR) |
| ko-uid / pr-uid / ko-state / graphics-count / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — PrGraphicUtil-only jattach; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `graphics-count` still `0`; `region-stats` named; `display-visibility-value` `FULL`; `docking-state` `NORMAL`; `synch-kind` `FoR`; docks unchanged |

**Route:** Composer headed verify **in progress** on `9b9c9dc`. **Next:** remaining TUTORIALS after PrGraphicUtil — **not** Pass.
