...
|---|---|
| WeasisWin `print-film` / `print-action` / `print-state` | **pending** — `name=print-state`; after Reed load `print-state` `getText` `none` |
| Film Session N-CREATE | **pending** — after `print-film`, `print-state` `film-session` (jattach `getText`, not OCR); no print SCP |
| N-ACTION Print type 1 | **pending** — after `print-action`, `print-state` `print`; no print SCP |
| send-state / sel-state / graphics-count / voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — print-only jattach; `send-state` still `none`; `sel-state` still `none`; `graphics-count` still `0`; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** Composer headed verify **in progress** on `ea11210`. **Next:** remaining TUTORIALS after DICOM Print — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_ea11210_print.png`
