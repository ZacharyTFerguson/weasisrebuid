...
|---|---|
| GraphicsPane `sel-sample` / `select-graphic` / `delete-graphic` / `sel-state` | **pending** — `name=sel-state`; after Reed load `sel-state` `getText` `none`; `graphics-count` `0` |
| Sample then select | **pending** — after `sel-sample`, `graphics-count` `1` and `sel-state` `none`; after `select-graphic`, `sel-state` `selected` (jattach `getText`, not OCR) |
| Delete | **pending** — after `delete-graphic`, `sel-state` `deleted` and `graphics-count` `0` |
| voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — select/delete-only jattach; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** Composer headed verify **in progress** on `da1e703`. **Next:** remaining TUTORIALS after select/delete map — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_da1e703_sel.png`
