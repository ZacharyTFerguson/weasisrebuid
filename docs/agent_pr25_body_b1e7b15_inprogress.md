...
**HEAD:** `b1e7b15` (`Show R/Q left-mouse live named rq-state chrome.`). **T/W left-mouse pan ≠ winLevel** / `name=left-state` headed PASS on `2b25a0d` (Composer: mouse_left pan≠winLevel on ViewerToolBar **bound view**, NOT cal-view). **S/Z left-mouse sequence ≠ zoom** / `name=sz-state` headed PASS on `4acbfef` (Composer: chest `org.weasis.dicom.viewer2d.View2d` bound ViewerToolBar, NOT cal-view; cal-view `mouse_left` stays `winLevel`; `left-state` stays `none`). **P ImagePrint** `94405ae`, **Freeze** `4dd370e`, **Calibration** `f8f66da`, **Q/R** `a17cff1`, **ISO** `693601d`, **DICOM Print** `ea11210`, **send** `090fa16`, and prior chrome headed PASS (not re-scored). **R/Q left-mouse rotation ≠ contextMenu** / `name=rq-state` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass. CHECKLIST §6 Pass stays **empty**.

Full ledger: `docs/agent_pr25_body_b1e7b15_inprogress.md`.
...
**Jar:** `/tmp/weasis-headed-b1e7b15.jar` · **SHA:** `b1e7b15013d3583e519e71212a660f32f226f43d` · **Jar file:** `c01270a5a6cbe6b972043cce84cceffd076fc49e4ee0131808a8008de7550d2c` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-b1e7b15-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-4acbfef.jar` or S/Z/T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`left-r`**, **`left-q`**, **`rq-state`** on **ViewerToolBar** whose `boundView()` is `org.weasis.dicom.viewer2d.View2d` (View2dContainer DICOM 2D bar — **not** WeasisWin north bar bound to `cal-view`). Duplicate `left-r` / `left-q` / `rq-state` names may exist on the north bar; pick the bar whose `boundView()` is the chest View2d (same instance Composer used for S/Z `left-s` / `left-z`). Live `name=`, `doClick`, not OCR. After load, **`rq-state`** `getText` expect `none`. Click **`left-r`** expect `rq-state` `rotation` (chest `mouse_left` `rotation`; cal-view stays `winLevel`; **`left-state` still `none`**; **`sz-state` still `none`**). Click **`left-q`** expect `rq-state` `contextMenu` (chest `mouse_left` `contextMenu`; rotation ≠ contextMenu; cal-view still `winLevel`; **`left-state` / `sz-state` still `none`**). Do **not** click ACTION tokens `rotation` / `contextMenu` / `sequence` / `zoom` / `pan` / `winLevel` / `reset-rotation` / `reset-zoom` and do **not** click `left-t` / `left-w` / `left-s` / `left-z` / `window-close`. Confirm `left-state` / `sz-state` / `img-print-state` / `print-state` / `freeze-state` / `cal-state` / `qr-state` / `iso-state` / `send-state` still `none`; `graphics-count` still `0`. Do **not** persist calling-AE preference keys. No optical burn / Print SCP / PACS I/O / PrinterJob.

| Check | Stills-based result |
|---|---|
| ViewerToolBar `left-r` / `left-q` / `rq-state` on chest bound view (NOT cal-view) | **pending** — `name=rq-state`; after Reed load `rq-state` `getText` `none`; `boundView()` is selected/chest `org.weasis.dicom.viewer2d.View2d` |
| R rotation | **pending** — after `left-r`, `rq-state` `rotation` (jattach `getText`, not OCR); chest `mouse_left` `rotation`; cal-view stays `winLevel`; `left-state` / `sz-state` still `none` |
| Q contextMenu ≠ R rotation | **pending** — after `left-q`, `rq-state` `contextMenu`; chest `mouse_left` `contextMenu` (rotation ≠ contextMenu); cal-view still `winLevel`; `left-state` / `sz-state` still `none` |
| left-state / sz-state / img-print-state / print-state / freeze-state / cal-state / qr-state / iso-state / send-state / graphics-count untouched | **pending** — R/Q-only jattach; `left-state` still `none`; `sz-state` still `none`; `img-print-state` still `none`; `print-state` still `none`; `freeze-state` still `none`; `cal-state` still `none`; `qr-state` still `none`; `iso-state` still `none`; `send-state` still `none`; `graphics-count` still `0`; prior docks unchanged (not re-scored) |

**Route:** Composer headed verify **in progress** on `b1e7b15`. **Next:** remaining TUTORIALS after R/Q left-mouse — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_b1e7b15_rq.png`
