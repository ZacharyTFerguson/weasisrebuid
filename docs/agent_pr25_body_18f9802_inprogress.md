...
**HEAD:** `18f9802` (`Add ImageTool AutoLevels chrome that stretches min/max via AutoLevelsOp.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, and **Brightness** `e9ef165` headed PASS (prior chrome not re-scored). **AutoLevels** / `name=autolevels` — Composer headed proof **in progress** (Reed chest W/L-painted already **0–255** → identity expected). Full TUTORIALS.md walkthrough is still **not** Pass.
...
| **`autolevels` / AutoLevels** on Image chrome | **pending** — jattach `name=autolevels` |
| Live toggle **AutoLevels** on (identity; no stretch) | **pending** — mean abs diff **0**; corr **1.0**; source **1929×2207** |
| Live toggle **AutoLevels** off (restore) | **pending** — corr **1.0** |
| **brightness** / **crop** / **window** / **flip** / **filter** untouched | **pending** — brightness must stay off (no +48 lift) |

**Route:** Composer headed verify running on `18f9802`. **Next:** Mask/Shutter after Pass — **not** Pass.
