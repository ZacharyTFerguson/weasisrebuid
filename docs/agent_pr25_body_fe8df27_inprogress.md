...
**HEAD:** `fe8df27` (`Show File Preferences live catalog dialog with named tree and OK chrome.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, **Mask** `3355cda`, **Shutter** `b00cd4d`, **DcmHeader Dump** `0d2f39f`, **DicomFieldsView** `eefe975`, **Export DICOM** `363f172`, **Help shortcuts/resources** `e88e780`, **Screenshot** `fc115d3`, **Import DICOM** `a569b23`, and **Help About** `17df6d0` headed PASS (prior chrome not re-scored). **File Preferences** / `name=file-preferences` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
**Jar:** `/tmp/weasis-headed-fe8df27.jar` · **SHA:** `fe8df27b039459920f21c9dd01d5aeefb10387aa` · **Jar file:** `b223cab8fc3463f94f4d71b0cdfb23c65bbe426298eb610fa01cbb5cb8faeed9` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-fe8df27-fresh`; prior headed JVMs killed; **did not reuse** About/Import/Screenshot/Help shortcuts/Export/Fields/Dump/ImageTool jars.
...
|---|---|
| File `file-preferences` / `Preferences` | **pending** — dialog title `Preferences`; `name=preferences` |
| Tree `pref-tree` | **pending** — includes General |
| Buttons `pref-ok` / `pref-cancel` / `pref-reset` | **pending** — named OK / Cancel / Restore defaults |
| About / Import / Screenshot / Export / ImageTool / Dump / Fields untouched | **pending** — Preferences-only jattach |

**Route:** Composer headed verify **in progress** on `fe8df27`. **Next:** remaining TUTORIALS after Preferences — **not** Pass.
