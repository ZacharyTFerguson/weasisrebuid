...
|---|---|
| WeasisWin `iso-write` / `iso-dicomdir` / `iso-state` | **pending** — `name=iso-state`; after Reed load `iso-state` `getText` `none` |
| ISO writeManifest | **pending** — after `iso-write`, `iso-state` `manifest` (jattach `getText`, not OCR); no optical burn |
| DICOMDIR include | **pending** — after `iso-dicomdir`, `iso-state` `DICOMDIR` |
| print-state / send-state / sel-state / graphics-count / voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **pending** — ISO-only jattach; `print-state` still `none`; `send-state` still `none`; `sel-state` still `none`; `graphics-count` still `0`; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** Composer headed verify **in progress** on `693601d`. **Next:** remaining TUTORIALS after ISO writer — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_693601d_iso.png`
