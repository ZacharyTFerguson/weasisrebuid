...
**HEAD:** `eefe975` (`Dock DicomFieldsView limited/search onto the explorer selected series.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, **Mask** `3355cda`, **Shutter** `b00cd4d`, and **DcmHeader Dump** `0d2f39f` headed PASS (prior chrome not re-scored). **DicomFieldsView** `eefe975` headed PASS. Full TUTORIALS.md walkthrough still **not** Pass. Full TUTORIALS.md walkthrough is still **not** Pass.
