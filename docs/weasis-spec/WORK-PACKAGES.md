...
| 6 | Docking Frames, FoR+manual sync, lens, 2D 3D-cursor | FoR vs MANUAL MX-14; docking-frames jar on `felix.auto.start.10` (not Active until FlatLaf extras) |
...
WP-0–4 Have is recorded in `docs/loops/TEAM-B-SCORE-wp0-4.md`. This branch adds Have tests for MX-14 (FoR vs manual), MPR/MIP, fusion `targetViews`, special-SOP factories Active on `lb`, WP-11 Q/R + DICOMweb + STOW + ISO + print, and WP-12 Dicomizer overlay (`felix.extended.config.properties`) plus still/video/PDF/STL convert Have. Docking Frames is listed at start level 10 but stays Installed until FlatLaf extras is on the catalog. JOGL natives, headed §6 Pass, and remaining source-surface paths stay open. SCORE may only change Have boxes. §6 Pass stays empty.
