...
| DICOM 2D / LUT / Zoom | 4 | W/L paints; zoom −200/−100; screenshot PNG/JPEG (headless-safe `ScreenshotDialog`); T/W/S/Z/R/C/Q/P keys; series/image nav; Ctrl-drag accelerate; Alt+R/L/F; Ctrl+Space; Ctrl+/-; Tab layout; Lut/Reset chrome; `ViewCanvas`/`Canvas`; `ViewerPrefView` documented W/L + PR overlay tokens | tutorial |
...
| SR / AU / ECG / RT / SEG | 10 | factories + MIME; SR Content Sequence display; ECG millivolt leads; AU PCM play/pause/stop; RT STRUCT contours by Z; SEG overlay tool/factory/prefs; `SegRegionLocator`; `UISegmentationVolumeBuildExecutor` | tutorial |
...
| `weasis://` `$dicom:get` `$dicom:rs` | 13 | protocol parser; base-explorer `AbstractThumbnailList` / `JIThumbnailListPane`; `JIThumbnailCache` URI LRU; `DefaultThumbnailList` / `AThumbnailListPane`; `ThumbnailList` / `IThumbnailListPane` / `AThumbnailModel`; `ThumbnailRenderer` / `ThumbnailIcon` | tutorial |
