...
**HEAD:** `eefe975` (`Dock DicomFieldsView limited/search onto the explorer selected series.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, **Mask** `3355cda`, **Shutter** `b00cd4d`, and **DcmHeader Dump** `0d2f39f` headed PASS (prior chrome not re-scored). **DicomFieldsView** / `name=DICOM Fields` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
**Jar:** `/tmp/weasis-headed-eefe975.jar` · **SHA:** `eefe9752e1871558bb30229232084b11c2751a8c` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-eefe975-fresh`; prior headed JVMs killed; **did not reuse** Dump/ImageTool jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D** → explorer left dock **`DICOM Fields`** (`name=DICOM Fields`) bound to the selected/imported series → jattach **`limited`**, **`tagSearch`**, **`tagViews`**, **`tagTable`**, **`tagDocument`** (not OCR). Limited on → search Patient → uncheck Limited for all dump. Do **not** click ImageTool / `dumpHeader`.

| Check | Stills-based result |
|---|---|
| Explorer dock `DICOM Fields` / `explorer-fields-split` | **pending** — jattach `name=DICOM Fields` |
| Limited clinical tags on Reed | **pending** — PatientName `Reed^Alice`; PatientID `XR-CHEST-001`; Modality `DX`; Rows/Columns **2207**/**1929**; WW **65535**; no PixelData |
| `tagSearch` `Patient` filters table+document | **pending** — PatientName/ID remain; Modality hidden |
| Uncheck `limited` all-dump | **pending** — PixelData `[OW]` |
| ImageTool / `dumpHeader` / `headerDump` untouched | **pending** — chrome false; raster **1929×2207** |

**Route:** Composer headed verify running on `eefe975`. **Next:** remaining TUTORIALS after DicomFieldsView — **not** Pass.
