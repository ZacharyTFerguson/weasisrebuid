...
**HEAD:** `d69ec46` (`Show dicom:rs live named rs-state chrome.`). **i18n lang-pref en ≠ fr** / `name=lang-state` headed PASS on `4c6297c` (Composer: WeasisWin north `lang-en`/`lang-fr`; `lang-state` none→en→fr; NOT ViewerToolBar). **dicom:close series ≠ all** / `name=close-state` headed PASS on `14d91f0` (Composer: `org.weasis.dicom.explorer.DicomExplorer` `close-series`/`close-all`; `close-state` none→series→all; NOT `window-close`). **Alt+R/L/F** / `name=alt-state` headed PASS on `0136004`. **R/Q** `b1e7b15`, **S/Z** `4acbfef`, **T/W** `2b25a0d` headed PASS (chest ViewerToolBar, NOT cal-view). **P ImagePrint** `94405ae`, **Freeze** `4dd370e`, **Calibration** `f8f66da`, **Q/R** `a17cff1`, **ISO** `693601d`, **DICOM Print** `ea11210`, **send** `090fa16`, and prior chrome headed PASS (not re-scored). **dicom:rs QIDO ≠ WADO-RS** / `name=rs-state` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass. CHECKLIST §6 Pass stays **empty**.

Full ledger: `docs/agent_pr25_body_d69ec46_inprogress.md`. Sprint inventory: `docs/weasis-spec/TUTORIALS-SPRINT-INVENTORY.md`.





## Headed `d69ec46` — `dicom:rs` QIDO ≠ WADO-RS / `name=rs-state` (Composer)

**Jar:** `/tmp/weasis-headed-d69ec46.jar` · **SHA:** `d69ec4681cc8cf438ccda1c9d20c8c6045f37b69` · **Jar file:** `6e0d38a1b2f54da3b8f76f4635c8cbe44c0ddb2455108b1464e58bebb402e3df` · **Gogo:** `17179` · OSGi cache `/tmp/weasis-headed-cache-d69ec46-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-4c6297c.jar` or lang/close/Alt/R/Q/S/Z/T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`rs-qido`**, **`rs-wado`**, **`rs-state`** on **`org.weasis.dicom.explorer.DicomExplorer`** (same dock as `close-series` / `close-all` / `explorer-filter-hits`; live `name=`, `doClick`, not OCR) — **not** ViewerToolBar, **not** `cal-view`, **not** WeasisWin `lang-en` / `lang-fr`, **not** `close-series` / `close-all` / `window-close`. After load, **`rs-state`** `getText` expect `none`. Click **`rs-qido`** expect `rs-state` `qido` (`dicom:rs` QIDO `/studies` plan; **no PACS**; **`lang-state` / `close-state` / `alt-state` / `rq-state` / `sz-state` / `left-state` still `none`**). Click **`rs-wado`** expect `rs-state` `wado-rs` (WADO-RS instance URL; QIDO ≠ WADO-RS; no PACS). Do **not** invent preference keys. Do **not** persist calling-AE. Do **not** click `close-series` / `close-all` / `lang-en` / `lang-fr` / `file-preferences` / `pref-ok` / `window-close` / `help-shortcuts-close` / `alt-r` / `left-r` / RotationToolBar `90°`. Confirm `lang-state` / `close-state` / `alt-state` / `rq-state` / `sz-state` / `left-state` / `img-print-state` / `print-state` / `freeze-state` / `cal-state` / `qr-state` / `iso-state` / `send-state` still `none`; `graphics-count` still `0`. No PACS / Print SCP / ISO burn / PrinterJob. Series DnD CLocation split **not** this item (blocked). Dicomizer **not** this item (separate overlay). ViewerHub `/display` **not** this item (pacs-connector servlet, not desktop Felix). Cine / AU/ECG/SR/RT / Q/R calling-AE skipped.

| Check | Stills-based result |
|---|---|
| DicomExplorer `rs-qido` / `rs-wado` / `rs-state` (NOT cal-view ViewerToolBar / NOT lang-en) | **pending** — `name=rs-state`; after Reed load `rs-state` `none` |
| `dicom:rs` QIDO | **pending** — after `rs-qido`, `rs-state` `qido`; no PACS; lang/close/alt/rq/sz/left still `none` |
| WADO-RS ≠ QIDO | **pending** — after `rs-wado`, `rs-state` `wado-rs` (qido ≠ wado-rs) |
| lang-state / close-state / alt-state / rq-state / sz-state / left-state / img-print / print / freeze / cal / qr / iso / send / graphics-count untouched | **pending** — rs-only jattach; prior chrome not re-scored |

**Route:** Composer headed verify **in progress** on `d69ec46`. **Next:** remaining TUTORIALS after dicom:rs — **not** Pass.

Still path: `/opt/cursor/artifacts/headed_d69ec46_rs.png`


## Headed `4c6297c` — i18n lang-pref en ≠ fr / `name=lang-state` (Composer)

**Jar:** `/tmp/weasis-headed-4c6297c.jar` · **SHA:** `4c6297c28e2011b614338f06ac90af804f8ca029` · **Jar file:** `9df37a10da9211da2e0f4e6b0738c313791053d080194947cabf715d3bd64ba7` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-4c6297c-fresh`.

**Flow:** `dicom:get` Reed chest on **DICOM 2D**. jattach **`lang-en`** / **`lang-fr`** on **WeasisWin north bar** (NOT ViewerToolBar). `lang-state` `none` → `en` (`locale.lang.code` `en`) → `fr` (`locale.lang.code` `fr`); en ≠ fr. `close-state` / `alt-state` / `rq-state` / `sz-state` / `left-state` stay `none`.

| Check | Stills-based result |
|---|---|
| WeasisWin `lang-en` / `lang-fr` / `lang-state` (NOT ViewerToolBar) | **OK** — `lang-en_not_viewer_toolbar` true |
| lang-en | **OK** — `lang-state` `en`; `locale.lang.code` `en` |
| lang-fr ≠ lang-en | **OK** — `lang-state` `fr`; `locale.lang.code` `fr` |
| close-state / alt-state / rq-state / sz-state / left-state untouched | **OK** — all `none`; `graphics-count` `0` |

**Route:** i18n lang-pref en ≠ fr / `lang-state` **headed OK** on `4c6297c`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_4c6297c_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_4c6297c_lang.png" width="720" />


## Headed `14d91f0` — `dicom:close` series ≠ all / `name=close-state` (Composer)

**Jar:** `/tmp/weasis-headed-14d91f0.jar` · **SHA:** `14d91f066ddc66b42e767e902ed667189323644c` · **Jar file:** `d02da07ee3bf829f69fb4d9632c58a3f0a404731b60968ec0dda34512f44506f` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-14d91f0-fresh`.

**Flow:** `dicom:get` Reed chest on **DICOM 2D**. jattach on **`org.weasis.dicom.explorer.DicomExplorer`** (`close-series` / `close-all` / `close-state` / `explorer-filter-hits`) — **not** ViewerToolBar, **not** `window-close`. `close-state` `none` → `close-series` → `series` → `close-all` → `all`; model instances 1→0; `alt-state` / `rq-state` / `sz-state` / `left-state` stay `none`.

| Check | Stills-based result |
|---|---|
| DicomExplorer close chrome (NOT cal-view / NOT window-close) | **OK** — `explorer_class` `org.weasis.dicom.explorer.DicomExplorer` |
| `dicom:close -s` series | **OK** — after `close-series`, `close-state` `series`; instances `0`; filter hits `0` |
| `dicom:close -a` all ≠ series | **OK** — after `close-all`, `close-state` `all` (series ≠ all) |
| alt-state / rq-state / sz-state / left-state / other workflow chrome untouched | **OK** — all `none`; `graphics-count` `0` |

**Route:** dicom:close series ≠ all / `close-state` **headed OK** on `14d91f0`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_14d91f0_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_14d91f0_close.png" width="720" />


## Headed `0136004` — Alt+R/L/F quarter-rotate ≠ flip / `name=alt-state` (Composer)

**Jar:** `/tmp/weasis-headed-0136004.jar` · **SHA:** `01360042956f3f75eae05008a74b9557334a4e4e` · **Jar file:** `eb18e97de5c2762162c8bdd89833aec76e081c6d1b8e7079207bd088ab21b7de` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-0136004-fresh`; prior headed JVMs killed.

**Flow:** `dicom:get` Reed chest on **DICOM 2D**. jattach **`alt-r`** → **`alt-l`** → **`alt-f`** on chest-bound `ViewerToolBar` (`org.weasis.dicom.viewer2d.View2d`, not `cal-view`). Sequence: `alt-state` `none` → `90` → `0` → `flip`; chest rotation 90→0; chest flip true after `alt-f`; cal-view rotation 0 and flip false throughout; `rq-state` / `sz-state` / `left-state` stay `none`.

| Check | Stills-based result |
|---|---|
| `alt-r` / `alt-l` / `alt-f` / `alt-state` on chest bound view | **OK** — `cal_view_distinct` true |
| Alt+R +90 | **OK** — `alt-state` `90`; chest rotation `90`; cal rotation `0` |
| Alt+L −90 ≠ Alt+R | **OK** — `alt-state` `0`; chest rotation `0` |
| Alt+F flip | **OK** — `alt-state` `flip`; chest flip true; cal flip false |
| rq-state / sz-state / left-state / other chrome untouched | **OK** — all `none`; `graphics-count` `0` |

**Route:** Alt+R/L/F / `alt-state` **headed OK** on `0136004`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_0136004_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_0136004_alt.png" width="720" />


## Headed `b1e7b15` — R/Q left-mouse rotation ≠ contextMenu / `name=rq-state` (Composer)

**Jar:** `/tmp/weasis-headed-b1e7b15.jar` · **SHA:** `b1e7b15013d3583e519e71212a660f32f226f43d` · **Jar file:** `c01270a5a6cbe6b972043cce84cceffd076fc49e4ee0131808a8008de7550d2c` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-b1e7b15-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-4acbfef.jar` or S/Z/T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest on **DICOM 2D**. jattach **`left-r`**, **`left-q`**, **`rq-state`** on chest-bound `ViewerToolBar` (`org.weasis.dicom.viewer2d.View2d`, not `cal-view`). `doClick`, not OCR. After load `rq-state` `none`. Click **`left-r`** → `rotation` (chest `mouse_left` `rotation`; cal-view `winLevel`; `left-state`/`sz-state` `none`). Click **`left-q`** → `contextMenu` (chest `mouse_left` `contextMenu`; rotation ≠ contextMenu).

| Check | Stills-based result |
|---|---|
| ViewerToolBar `left-r` / `left-q` / `rq-state` on chest bound view (NOT cal-view) | **OK** — chest `boundView()` `org.weasis.dicom.viewer2d.View2d`; `cal_view_distinct` true |
| R rotation | **OK** — after `left-r`, `rq-state` `rotation`; chest `mouse_left` `rotation`; cal-view `winLevel`; `left-state`/`sz-state` `none` |
| Q contextMenu ≠ R rotation | **OK** — after `left-q`, `rq-state` `contextMenu`; chest `mouse_left` `contextMenu` |
| left-state / sz-state / img-print-state / print-state / freeze-state / cal-state / qr-state / iso-state / send-state / graphics-count untouched | **OK** — R/Q-only jattach; all `none`; `graphics-count` `0` |

**Route:** R/Q left-mouse rotation ≠ contextMenu / `rq-state` **headed OK** on `b1e7b15`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_b1e7b15_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_b1e7b15_rq.png" width="720" />


## Headed `4acbfef` — S/Z left-mouse sequence ≠ zoom / `name=sz-state` (Composer)

**Jar:** `/tmp/weasis-headed-4acbfef.jar` · **SHA:** `4acbfef18f22984ced35ecb7f56d6099c209e515` · **Jar file:** `769a1df7eba388d24025e06f8feac44587aff1bf838199d4da679d291500fed3` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-4acbfef-fresh`; prior headed JVMs killed; **did not reuse** `/tmp/weasis-headed-2b25a0d.jar` or T/W/ImagePrint/Freeze/Calibration/Q/R/ISO/print/send jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`left-s`**, **`left-z`**, **`sz-state`** on **ViewerToolBar** whose `boundView()` is `org.weasis.dicom.viewer2d.View2d` (View2dContainer DICOM 2D bar — **not** WeasisWin north bar bound to `cal-view`). Live `name=`, `doClick`, not OCR. After load, **`sz-state`** `none`. Click **`left-s`** → `sequence` (chest `mouse_left` `sequence`; cal-view stays `winLevel`; **`left-state` `none`**). Click **`left-z`** → `zoom` (chest `mouse_left` `zoom`; sequence ≠ zoom; cal-view still `winLevel`; **`left-state` `none`**).

| Check | Stills-based result |
|---|---|
| ViewerToolBar `left-s` / `left-z` / `sz-state` on chest bound view (NOT cal-view) | **OK** — chest `boundView()` `org.weasis.dicom.viewer2d.View2d`; `cal_view_distinct` true |
| S sequence | **OK** — after `left-s`, `sz-state` `sequence`; chest `mouse_left` `sequence`; cal-view `winLevel` |
| Z zoom ≠ S sequence | **OK** — after `left-z`, `sz-state` `zoom`; chest `mouse_left` `zoom` |
| left-state / img-print-state / print-state / freeze-state / cal-state / qr-state / iso-state / send-state / graphics-count untouched | **OK** — S/Z-only jattach; all `none`; `graphics-count` `0` |

**Route:** S/Z left-mouse sequence ≠ zoom / `sz-state` **headed OK** on `4acbfef`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_4acbfef_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_4acbfef_sz.png" width="720" />


## Headed `2b25a0d` — T/W left-mouse pan ≠ winLevel / `name=left-state` (Composer)

**Jar:** `/tmp/weasis-headed-2b25a0d.jar` · **SHA:** `2b25a0dea421523d08ce00c6270b832641448345` · **Jar file:** `66a73b566e69db590accc7e7a4b4efc53a83993e5c5207abddccdf6f59ea67ac` · **Gogo:** `17279` · OSGi cache `/tmp/weasis-headed-cache-2b25a0d-fresh`; prior headed JVMs killed; **did not reuse** ImagePrint/Freeze/Calibration/Q/R/ISO/print/send/select-delete/VOI/filter/Reset/interp/apply-PR/KO jars.

**Flow:** `dicom:get` Reed chest (`/opt/cursor/artifacts/chest_pa.dcm`) on **DICOM 2D**. DummyViewerPlugin **off**. jattach **`left-t`**, **`left-w`**, **`left-state`** (ViewerToolBar on WeasisWin north, live `name=`, `doClick`, not OCR). After load, **`left-state`** `getText` expect `none`. Click **`left-t`** expect `left-state` `pan` (mouse left `pan`; ViewerToolBar `getSelected` `pan`). Click **`left-w`** expect `left-state` `winLevel` (mouse left `winLevel`; distinct from pan). Do **not** click ACTION token buttons named `pan` / `winLevel` / `zoom` / `rotation` / `sequence` / `reset-winLevel`. Confirm `img-print-state` still `none` (do not click `img-page` / `img-print`), `print-state` still `none` (do not click `print-film` / `print-action`), `freeze-state` still `none` (do not click `freeze-params` / `freeze-image`), `cal-state` still `none` (do not click `cal-line` / `cal-apply`), `qr-state` still `none` (do not click `qr-find` / `qr-move`), `iso-state` still `none` (do not click `iso-write` / `iso-dicomdir`), `send-state` still `none` (do not click `send-cstore` / `send-stow`), `sel-state` still `none` (do not click `sel-sample` / `select-graphic` / `delete-graphic`), `graphics-count` still `0` (do not click `measure-rect`), `voi-lut-state` still `none` (do not click `voi-table` / `voi-sigmoid`), `explorer-filter-hits` still named (do not type query), `reset-state` still `none` (do not click `reset-winLevel` / `reset-all`), `interp-shape` still `none`, `pr-mapped` still `none`, `ko-uid` / `pr-uid` still `none`, `ko-state` still `none`, `region-stats` named, `display-visibility-value` `FULL`, `docking-state` `NORMAL`, `synch-kind` `FoR`, `lens` east, `mini-tool` west, `histogram` south, `pixel-info` present. Do **not** click Lut Sharpen `filter` / `inverseLut` / `voi-table` / `voi-sigmoid` / `star` / `ko-filter` / `save-ko` / `save-pr` / `apply-pr` / `apply-interp` / `reset-winLevel` / `reset-all` / `reset-zoom` / `reset-rotation` / Zoom `2x` / `90°` / `display-full` / `display-minimal` / `display-hidden` / `window-close` / docking keys / lens / `synch-manual` / `crosshair` / Mini Tool / Histogram RGB / Preferences / About / Import / Screenshot / Help / Export / ImageTool / `dumpHeader` / Fields / Measure D/A/Y / `measure-rect` / `sel-sample` / `select-graphic` / `delete-graphic` / `send-cstore` / `send-stow` / `print-film` / `print-action` / `iso-write` / `iso-dicomdir` / `qr-find` / `qr-move` / `cal-line` / `cal-apply` / `freeze-params` / `freeze-image` / `img-page` / `img-print`. Do **not** persist calling-AE preference keys. No optical burn / Print SCP / PACS I/O / PrinterJob.

| Check | Stills-based result |
|---|---|
| ViewerToolBar `left-t` / `left-w` / `left-state` | **OK** — `name=left-state`; after Reed load `left-state` `getText` `none` |
| T pan | **OK** — after `left-t` `doClick`, `left-state` `pan`; bound view `mouse_left` `pan` |
| W window/level ≠ T pan | **OK** — after `left-w` `doClick`, `left-state` `winLevel`; `mouse_left` `winLevel` (pan ≠ winLevel) |
| img-print-state / print-state / freeze-state / cal-state / qr-state / iso-state / send-state / sel-state / graphics-count / voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **OK** — T/W-only jattach; `img-print-state` still `none`; `print-state` still `none`; `freeze-state` still `none`; `cal-state` still `none`; `qr-state` still `none`; `iso-state` still `none`; `send-state` still `none`; `sel-state` still `none`; `graphics-count` still `0`; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** T/W left-mouse pan ≠ winLevel / `left-state` **headed OK** on `2b25a0d`. **Next:** remaining TUTORIALS — **not** Pass.

Verify JSON: `/opt/cursor/artifacts/headed_2b25a0d_verify.json` (`misses`: `[]`).

<img src="/opt/cursor/artifacts/headed_2b25a0d_left.png" width="720" />
...
| Check | Stills-based result |
|---|---|
| WeasisWin `img-page` / `img-print` / `img-print-state` | **OK** — `name=img-print-state`; after Reed load `img-print-state` `getText` `none` |
| Page (annotations off) | **OK** — after `img-page`, `img-print-state` `page` (jattach `getText`, not OCR); lastPrint raster; showingAnnotations false; no PrinterJob |
| Printable ≠ page (annotations on) | **OK** — after `img-print`, `img-print-state` `printable`; showingAnnotations true; lastPrint raster |
| print-state / freeze-state / cal-state / qr-state / iso-state / send-state / sel-state / graphics-count / voi-lut-state / explorer-filter-hits / reset-state / interp-shape / pr-mapped / ko-uid / pr-uid / ko-state / region-stats / LayerItem / Display 3-state / docking keys / FoR / Lens east / Mini Tool west / Histogram south / PixelInfo untouched | **OK** — ImagePrint-only jattach; `print-state` still `none` (DICOM Print untouched); `freeze-state` still `none`; `cal-state` still `none`; `qr-state` still `none`; `iso-state` still `none`; `send-state` still `none`; `sel-state` still `none`; `graphics-count` still `0`; `voi-lut-state` still `none`; `reset-state` still `none`; `interp-shape` still `none`; `pr-mapped` still `none`; `ko-uid` / `pr-uid` still `none`; `ko-state` still `none`; `explorer-filter-hits` named; prior docks unchanged (not re-scored) |

**Route:** P ImagePrint page ≠ printable / `img-print-state` **headed OK** on `94405ae`. **Next:** remaining TUTORIALS — **not** Pass.

<img src="/opt/cursor/artifacts/headed_94405ae_imgprint.png" width="720" />
