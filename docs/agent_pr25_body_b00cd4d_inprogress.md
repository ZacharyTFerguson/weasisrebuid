...
**HEAD:** `b00cd4d` (`Add ImageTool Shutter chrome that blacks an eighth-inset box via ShutterOp.`). **Flip** `004f59d`, **Sharpen** `3844bd4`, **Window** `3091f5e`, **Crop** `f88ec68`, **Brightness** `e9ef165`, **AutoLevels** `18f9802`, and **Mask** `3355cda` headed PASS (prior chrome not re-scored). **Shutter** / `name=shutter` — Composer headed proof **in progress**. Full TUTORIALS.md walkthrough is still **not** Pass.
...
| **`shutter` / Shutter** on Image chrome | **pending** — jattach `name=shutter` |
| Live toggle **Shutter** on (full raster; ring w/8…w/4 not Mask-zeroed) | **pending** — source **1929×2207**; mean abs diff ~**61**; corr ~**0.10** |
| Live toggle **Shutter** off (restore) | **pending** — corr **1.0**; mean abs diff **0** |
| **mask** / **autolevels** / **brightness** / **crop** / **window** / **flip** / **filter** untouched | **pending** |

**Route:** Composer headed verify running on `b00cd4d`. **Next:** remaining TUTORIALS — **not** Pass.
