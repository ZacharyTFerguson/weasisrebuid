...
/**
 * Newest-on-top rules; modalities OR; archive or ALL. Default WEASIS when {@code viewer} omitted.
 */
public final class ViewerSelectionRules {
  ...
  static boolean matches(Rule rule, DisplayQuery query) {
    ...
    for (String m : rule.modalities()) {
      for (String q : query.modalitiesInStudy()) {
  ...
  public static DisplayQuery.Viewer parseViewer(String raw) {
    if (raw == null || raw.isBlank()) {
      return DisplayQuery.Viewer.WEASIS;
    }
    return DisplayQuery.Viewer.valueOf(raw.trim().toUpperCase(Locale.ROOT));
  }
}
