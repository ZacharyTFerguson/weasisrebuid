...
public class DicomVideoElement extends AudioVideoElement implements DicomElement, FileExtractor {
  private final DcmMediaReader mediaIO;
  private File extractFile;

  public DicomVideoElement(DcmMediaReader mediaIO) {
    this.mediaIO = mediaIO;
    if (mediaIO != null) {
      setMediaURI(mediaIO.getUri());
      setMimeType(DicomMime.VIDEO_DICOM);
